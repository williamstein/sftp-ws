...
gulp.task(
  ...
  gulp.series("lib", function () {
    ...
    var sources = gulp.src(input.sources, { base: "." }).pipe(ts.lib()).js;

    var metadata = gulp.src(input.metadata).pipe(
    ...
    return merge(sources, metadata, files).pipe(gulp.dest(output.packages));
  })
);
...
//       },
    ...
    return gulp
      .src(output.web + "/sftp.js")
      .pipe(
        rename(function (path) {
          return (path.basename = "sftp.min");
        })
      )
...
gulp.task("build", gulp.series("lib", "package", "web"));
